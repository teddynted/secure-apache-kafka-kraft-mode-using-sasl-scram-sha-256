...
KRAFT_ADVERTISED_LISTENERS=$(cat /opt/kafka/config/kraft/server.properties | grep -c "advertised.listeners=SASL_SSL://$PUBLIC_DNS_NAME:9092")
...
#   password='$1';
# EOF'
# sudo /opt/kafka/bin/kafka-metadata-quorum.sh --bootstrap-server ip-172-31-20-235.eu-west-1.compute.internal:9092 describe --status
# sudo /opt/kafka/bin/kafka-metadata-quorum.sh --bootstrap-server ip-172-31-20-235.eu-west-1.compute.internal:9092 --command-config /opt/kafka/config/kraft/client.properties describe --status
# sudo /opt/kafka/bin/kafka-topics.sh --create --bootstrap-server ip-172-31-20-235.eu-west-1.compute.internal:9092 --replication-factor 1 --partitions 3 --topic testtopic --if-not-exists --command-config /opt/kafka/config/kraft/client.properties
# sudo /opt/kafka/bin/kafka-topics.sh --bootstrap-server ip-172-31-20-235.eu-west-1.compute.internal:9092 --list --command-config /opt/kafka/config/kraft/client.properties
...
# sudo /opt/kafka/bin/kafka-acls.sh --bootstrap-server ip-172-31-20-235.eu-west-1.compute.internal:9092 --list --cluster
...
sudo /opt/kafka/bin/kafka-configs.sh --bootstrap-server ip-172-31-20-235.eu-west-1.compute.internal:9092 --command-config /opt/kafka/config/kraft/client.properties --alter --add-config 'SCRAM-SHA-256=[password='Passw0rd123']' --entity-type users --entity-name admin
sudo /opt/kafka/bin/kafka-configs.sh --bootstrap-server $PRIVATE_IP_ADDRESS:9092 --command-config /opt/kafka/config/kraft/client.properties --alter --add-config 'SCRAM-SHA-256=[password='$1']' --entity-type users --entity-name broker
...
sudo /opt/kafka/bin/kafka-acls.sh --bootstrap-server $PRIVATE_IP_ADDRESS:9092 --command-config /opt/kafka/config/kraft/client.properties --add --allow-principal "User:broker" --operation ClusterAction --cluster
...
sudo /opt/kafka/bin/kafka-metadata-quorum.sh --bootstrap-controller $PRIVATE_IP_ADDRESS:9092 --command-config /opt/kafka/config/kraft/client.properties describe --status
fi
