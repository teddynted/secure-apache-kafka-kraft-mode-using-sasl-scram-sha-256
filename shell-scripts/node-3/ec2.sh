...
ssh -i $private_key -o "StrictHostKeyChecking no" ec2-user@$public_dns_name "bash -s" < ./shell-scripts/node-3/ec2-commands.sh $password $username $region $private_dns_name $public_ip_address $private_dns_name_node_1 $private_dns_name_node_2 $NODE_THREE_TOPIC $OFFSETS_TOPIC_REPLICATION_FACTOR $DEFAULT_REPLICATION_FACTOR $NODE_ID