...
SECRET_JSON=$(aws secretsmanager get-secret-value --secret-id "KafkaBrokerSaslScram256" --region "eu-west-1" --query SecretString --output text)
...
sudo sed -i s/controller.quorum.voters=1@localhost:9093/controller.quorum.voters=1@$PRIVATE_DNS_NAME_NODE:9093,2@$PRIVATE_DNS_NAME_NODE_2:9093,3@$PRIVATE_DNS_NAME_NODE_3:9093/ /opt/kafka/config/kraft/server.properties
...
# sudo /opt/kafka/bin/kafka-configs.sh --bootstrap-server $PRIVATE_DNS_NAME:9092 --command-config /opt/kafka/config/kraft/client.properties --alter --add-config 'SCRAM-SHA-256=[iterations=4096,password='$PASSWORD']' --entity-type users --entity-name admin
